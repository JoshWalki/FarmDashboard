...
`;
  ...
  getFieldConditionIcons(field) {
    ...
    let fertLevel = field.fertilizationLevel || 0;
    let fertPercent = (fertLevel / 2) * 100;
    let fertColor = fertLevel >= 2 ? "success" : fertLevel >= 1 ? "warning" : "danger";
    let fertDisplay = `${fertLevel}/2`;

    // Precision Farming Override
    if (field.isPrecisionFarming) {
      if (field.isScanned) {
        fertDisplay = field.nitrogenText || fertDisplay;
        
        // Calculate true percentage based on Nitrogen kg/ha
        if (field.targetNitrogen && field.targetNitrogen > 0) {
          fertPercent = Math.min(100, Math.max(0, (field.nitrogenLevel / field.targetNitrogen) * 100));
        } else {
          fertPercent = field.nitrogenLevel > 0 ? 100 : 0;
        }
        
        // Adjust colors
        fertColor = field.needsFertilizer ? (field.nitrogenLevel === 0 ? "danger" : "warning") : "success";
      } else {
        fertDisplay = "Needs Scan";
        fertPercent = 0;
        fertColor = "danger";
      }
    }
    ...
    // --- 2. Lime (pH) ---
    let limeLevel = field.limeLevel || 0;
    let limePercent = limeLevel >= 1 ? 100 : 0;
    let limeColor = limeLevel >= 1 ? "success" : "danger";
    let limeDisplay = limeLevel >= 1 ? "Applied" : "Needed";

    // Precision Farming Override
    if (field.isPrecisionFarming) {
      if (field.isScanned) {
        limeDisplay = field.limeText || limeDisplay;
        
        // Calculate true percentage based on pH target
        if (field.targetPh && field.targetPh > 0) {
          limePercent = Math.min(100, Math.max(0, (field.phValue / field.targetPh) * 100));
        } else {
          limePercent = Math.min(100, Math.max(0, (field.phValue / 6.5) * 100));
        }
        
        // Adjust colors
        limeColor = field.needsLime ? (field.phValue < 5.5 ? "danger" : "warning") : "success";
      } else {
        limeDisplay = "Needs Scan";
        limePercent = 0;
        limeColor = "danger";
      }
    }
    ...
    conditions.forEach((condition) => {
      ...
      // Weeds bar operates backwards in the original code, so we preserve that
      let barWidth = condition.percent;
      if (condition.name === "Weeds") {
        barWidth = condition.percent === 0 ? 100 : Math.max(0, 100 - condition.percent);
      }

      // Slightly smaller font so the kg/ha text doesn't push the card layout
      const fontStyle = (condition.name === "Fertilizer" || condition.name === "Lime") 
      ...
      html += `
        <div class="col-6">
          <div class="d-flex align-items-center mb-1">
            ...
            <strong class="ms-auto text-${condition.color}" ${fontStyle}>${condition.display}</strong>
          </div>
          <div class="progress" style="height: 4px;">
            <div class="progress-bar bg-${condition.color}" style="width: ${barWidth}%"></div>
          </div>
        </div>
      `;
    });
